...
description: 将符合规范的简历 Markdown 自动渲染成 A4 双页排版的 HTML（CSS 模板即 PDF-ready，HTML 自身可直接出 PDF）。使用场景：用户提供写好的简历 Markdown 文件，希望生成可直接打印 PDF 的精美 HTML 输出。会读取 md 结构、套用预设 CSS 模板、按 A4 自动分页。
---

# Resume Markdown → HTML 渲染 Skill

把规范的简历 Markdown 自动转换为 A4 双页排版的 HTML 文件。CSS 模板即 PDF-ready：浏览器打开 HTML 的样子就是 PDF 的样子，不需要再写 `@media print` 双套样式。
...
- `/resume-md-to-html <input.md>` → 仅生成 HTML（默认）
- `/resume-md-to-html <input.md> pdf` → 生成 HTML 后，自动调用 `html-to-pdf` skill 出 PDF（默认 screen 模式）
- `/resume-md-to-html <input.md> pdf print` → 生成 HTML + PDF（print 模式，遵循 @media print）
...
3. **生成** HTML，按以下顺序组装（不要省略任何 section）：
   ...
   <div class="page">  <!-- 第 1 页 -->
     <div class="page-body">  <!-- safe center 让内容在垂直方向居中 -->
   ...
   <div class="page page-second">  <!-- 第 2 页：加 page-second 类，顶部留白更舒适 -->
     <div class="page-body">
...
6. **可选 PDF**（如果用户 args 里带 `pdf`）：调用 `html-to-pdf` skill 出 PDF。
   - 本模板的 HTML 自身即 PDF-ready；`pdf` 后无第三参数走 screen 模式、显式 `print` 走 print 模式——两种模式产物一致（strip 脚本对新模板是 no-op）
   - 详见 `html-to-pdf` skill 的「渲染模式」章节
...
- **A4 单页内容区：** `.page { width: 200mm; min-height: 287mm; padding: 16mm; }` —— 对应 `@page` 留 5mm 后的可打印区域
- **flex 列 + 居中：** `.page { display: flex; flex-direction: column; }` 把 `.page-body` 撑满；`.page-body { flex: 1; display: flex; flex-direction: column; justify-content: safe center }` 让内容在垂直方向居中。`safe center` 在内容溢出时自动回退到 `start`，避免被裁切
- **第 2 页顶部留白：** `.page-second { padding-top: 22mm }` —— 比首页 padding 稍多，避免第 2 页 section 紧贴页眉
- **打印分页：** `@page { size: A4; margin: 5mm; }` + `.page { page-break-after: always; }` 让每张 div 强制对应一张物理 A4，5mm 白色外边距模拟屏幕视图的视觉间距
...
- [ ] 第 1 页项目一没有溢出（A4 高度内）
- [ ] 若 args 含 `pdf`，调用 `html-to-pdf` skill 出 PDF，并验证 `kMDItemNumberOfPages = 2`
